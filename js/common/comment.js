if (
  window.location.href.includes("notice_detail") ||
  window.location.href.includes("notices_detail") ||
  window.location.href.includes("posts_detail") ||
  ...
  async function AddComment(pId = null, type = "comment") {
    ...
    const response = await fetch(
      `http://34.64.161.55:80/api/comments`,
      options
    );
