    ...
    for (let i = 0; i < 9; i++) {
        ...
        usersDiv.innerHTML += `
            <div class="col-3 p-4 m-2" onclick="checktoGoDetailPage(${Arr[i]?.userId})" 
                    ...
                    <div class="my-1" style="font-size: large"><b>${Arr[i]?.defaultInfo.name || "비회원"}</b></div>
                    <div class="my-1"><b>${Arr[i]?.defaultInfo.generation}기</b></div>
                    ...
                    <div class="my-1">
                        ...
                        <a href="${Arr[i]?.defaultInfo.github}" style="color: inherit;">
...
function sortArr(Arr, option) {
    if (option == "created") {
        ...
        for (let i = 0; i < Arr.length - 1; i++) {
            for (let j = i + 1; j < Arr.length; j++) {
                if (parseInt(Arr[i]?.defaultInfo.studentId) > parseInt(Arr[j]?.defaultInfo.studentId)) {
                    let temp = Arr[i];
                    Arr[i] = Arr[j];
                    Arr[j] = temp;
                }
